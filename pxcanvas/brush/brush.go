...
func Cursor(config apptype.PxCanvasConfig, brush apptype.BrushType, ev *desktop.MouseEvent, x, y int) []fyne.CanvasObject {
	...
	case Pixel:
		...
		buttom := canvas.NewLine(cursorColor)
		buttom.StrokeWidth = 3
		buttom.Position1 = fyne.NewPos(xOrigin, yOrigin+pxSize)
		buttom.Position2 = fyne.NewPos(xOrigin+pxSize, yOrigin+pxSize)

		objects = append(objects, left, top, right, buttom)
		return objects
	default:
		return objects
	}
